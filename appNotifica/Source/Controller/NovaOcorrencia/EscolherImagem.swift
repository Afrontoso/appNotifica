...
//  appNotifica
//
//  Created by IFB-BIOTIC-12 on 06/12/23.
//

import Foundation
import UIKit

class EscolhaImagem: NSObject, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        guard let image = info [.originalImage] as? UIImage else{
            fatalError("Espera-se uma imagem, mas foi dado o seguinte: \(info)")
        }
        
        returnoSelecionador?(image)
    }
}
